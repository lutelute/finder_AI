...
- 設定が有効でもtmuxが見つからなければ黙って通常セッションに落とします。「起動できない」より「永続でないが動く」が正しい失敗の仕方です。tmux未導入でトグルを押した場合は導入方法を案内します。
- `PersistentSessionsPanelController`（表示メニュー「永続セッションを管理…」）が残存セッションの一覧・選択終了・一括終了・フォルダを開くを提供します。フォルダはセッション名から戻せない（ハッシュ）ため、`#{session_path}`でtmux自身に答えさせます。一覧と終了は**永続化トグルに依存しません**。トグルを切った後に残ったセッションの掃除が、このパネルの主目的の一つだからです。終了対象は`finderai-`名義に限定し、ユーザー自身のtmuxセッションには触れません。
