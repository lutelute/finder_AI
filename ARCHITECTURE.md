...
- **選ぶ口はツールバーのボタンです。** メニューの奥にだけ置くと、在ることに気付かれません。ボタンはペインが持ちますが、選んだ結果は`onSelectTint`で窓へ渡します——ペインは自分の額縁しか塗れないので、そこで自分だけ塗るとタイトルバーや反対側のペインと色が食い違います。保存とメニューの印の引き直しは`onTintChanged`が一本で受けるので、ボタンから選んでもメニューから選んでも同じ道を通ります。
- **ボタンの名前はビューに持たせます。** `configureNavigationButton`は記号の`accessibilityDescription`を名前の出どころにしていますが、このボタンは絵を差し替えるので、そのままだと読み上げから名前が消えます。`setAccessibilityLabel`で明示します。
