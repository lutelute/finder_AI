...
`TerminalSessionsPanelController`（表示メニュー「Terminalセッションを管理…」⌘⌥T）が全セッションの俯瞰です。ドロワーは現在フォルダしか見せないため、複数ウインドウで方々にセッションを起こしたときの全体像はここに出ます。行の組み立ては`TerminalSessionsOverview.rows`（純関数）で、アプリ内セッション（開いた順）→tmux残存（パス順）と並べ、アプリ内で接続中の永続セッションは`tmux ls`にも載るので名前で重複排除します。フォルダはセッション名から戻せない（ハッシュ）ため、`#{session_path}`でtmux自身に答えさせます。一覧と終了は**永続化トグルに依存しません**。トグルを切った後に残ったセッションの掃除が、このパネルの主目的の一つだからです。終了対象はアプリ内セッションと`finderai-`名義のtmuxセッションに限定し、ユーザー自身のtmuxセッションには触れません。
