...
- **混ぜる色は地の明度に寄せ、彩度だけ上げます。** 明るい側は淡く、暗い側は深く。逆をやって実機で外しました——暗い地に明るい色を混ぜると、面の明度が上がって上の明るい字とのコントラストが落ちる（副文5.79→4.09）。それでいて色は出ない。**薄く混ぜるほど安全ではありません**。効くのは割合ではなく混ぜる色のほうで、いまはどちらも0.45で混ぜています。実際の比は`WindowTintContrastTests`が測ります——濃さの数字だけを縛っても何も守れません。
- **濃さは設定で動かせます**（`WorkspacePreferences.windowTintStrength`、15〜80%）。既定の45%は暗い側の副文が4.5を保つ上限で、そこから先は**読みやすさを進んで手放す側**です。何十枚と開く人にとって「どれがどれか」のほうが切実な場面があるので開けてありますが、既定は落とさない側に置きます。`double(forKey:)`は無いキーに0を返すので、`object(forKey:)`で「まだ選んでいない」と分けます——分けないと初回起動が必ず0（色なしと同じ）になります。
- **スライダーには6色の見本を添えます**（`WorkspaceTintStrengthPreview`）。数字だけでは決められず、確かめるたびに窓を見に行くのでは行ったり来たりになります。見本は小さな窓の形をしていて、色が乗る面の比率をそのまま写すので、**ファイル一覧には色が付かない**ことも手元で分かります。
- **上端に4ptの帯を出します。** 面の色は地に寄せてあるので遠目には弱い。帯は**混ぜない一色**（`barHex`、中間調）なので、小さい窓でも重なった窓でも残ります。色なしのときは高さ0にして、いまの見た目を一切崩しません。
- **タイトルバーの文字色はウインドウの外観が決めます。** 背景だけ塗っても文字は付いてきません。ウインドウに外観を掛けていなかったので、中身を暗くしてもタイトルバーだけ明るい素材＋暗い文字のままで、そこへ暗い色を敷いて文字が沈みました。`window.appearance`を中身に合わせます。子のビューは自分の外観を持つので、ターミナルの明るさは巻き込みません。
- **サイドバーの一覧は`.sourceList`をやめて`.inset`にしました。** `.sourceList`はvibrancyの地を自分で描くので、下に敷いた色が透けません。`NSClipView`の背景も別に切る必要があります——`NSScrollView.drawsBackground`を落としても、切り抜き側は不透明なまま残ります。
- **色を付ける先は「いま見ている窓」です。** `frontmostWindow`だけではkeyWindowが無いときに配列の先頭＝最初に作った窓へ落ち、見ていない窓に色が付きました。最後にkeyだったワークスペース窓を先に見ます。
