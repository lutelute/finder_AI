...
最終更新: 2026-09-02（Asia/Tokyo）

版ごとの記録を上から新しい順に積みます。**未実施項目を合格扱いにしません。** 下の
「v1.7追加検証」以降は、その時点の記録としてそのまま残してあります。テスト数など
当時の数字が今と違うのは、書き換えずに残しているためです。

## v1.39.0（2026-09-02、窓ごとの色）

- `swift test` が **510テスト／79 suites 成功**（1.38.2から+17件）。所要6.3秒。
- **描画を実物で確認済み。** 実機のウインドウには触れず、
  `bitmapImageRepForCachingDisplay` + `cacheDisplay` で額縁の塗りをPNGへ書き出して目視した。
  6色＋色なしが、ライト・ダークの両方で互いに区別でき、**ファイル一覧の地は灰のまま**
  であることを確認。使い捨てのテストなのでリポジトリには残していない。
- 実機のウインドウで触る確認（メニューから選ぶ、窓を並べる、再起動で色が戻る）は
  **未実施**。下の「まだユーザー操作で確認が必要」に追記した。

## v1.38.2（2026-09-02、自動テストのみ）

- `./scripts/run-tests.sh` が **493テスト／76 suites 成功**。所要6.0秒。
  リリーススクリプト検査（`test-release-scripts.sh`）も通過。
- ~~コンパイル警告0件~~ **これは誤り**。増分ビルドで再コンパイルされたファイルが
  無かっただけで、全体を組み直すと既存の警告が3か所出る
  （`WorkspaceMapView.swift` の `copiesOnScroll` 非推奨2件、
  `DrawerTerminalMountingTests.swift` の `Selector` 直書き、
  `TerminalSessionsOverviewTests.swift` の冗長な `#require`）。
  **警告の数を報告するなら、キャッシュを効かせないビルドで測ること。**
- ビルドは `~/Documents`（OneDrive の File Provider 配下）ではなく `/private/tmp` の
  複製で実行。同期がビルド中のソースを触ると
  `input file ... was modified during the build` で散発的に落ちるため。
- shellcheck が `scripts/*.sh` を通過。GitHub Actions の Tests ワークフローも直近8回すべて成功。
- インストール済み `/Applications/FinderAI.app` は 1.38.2（build 103、commit `423422f`）で、
  そのときの HEAD と一致。

**この回は実画面での確認を行っていません。** UIに触れる変更を入れていないためで、
下の「まだユーザー操作で確認が必要」はそのまま残っています。
...
- **ウインドウごとの色（1.39.0）**: 表示メニューから6色と「色なし」を選ぶ／窓を3枚以上
  並べて視線を走らせたときに見分けが付くか／⌘Qして起動し直したときに同じ窓へ同じ色が戻るか／
  ライトとダークを切り替えてもタイトルバーの色が付いてくるか
