...
## 6. フォルダを作らずにまとめる（グループ）

<img src="media/groups-drag.gif" width="880" alt="未分類の一覧から島へ引いてグループに入れ、島から島へ張り替える様子">

平らに並んだ何十ものフォルダを、**実体を一つも動かさずに**まとめられます。まとめた記述は、そのフォルダに置いた一枚のJSON（`.finderai-groups.json`）だけ。gitのパスもシンボリックリンクも壊れません。

- **入れる**: 一覧の見出しへ引く、マップの島へ引く、右クリックの「グループに入れる」
- **張り替える**: マップで点を掴んで別の島へ引く（`⌥`を押しながらなら、元にも残したまま増やす）
- **一つが複数のグループに属してよい**。マップではその点が島の境界に立ち、輪が所属の数で割れます
- **入れ子にできる**（`可視化 ∈ 研究`）。見出しの右クリックから

<img src="media/groups-map.gif" width="880" alt="マップで島を広げて中身を見る様子">

マップは「重なりを見る」ための表示です。島の高さは中身の量そのもので、入りきらないぶんは紙のほうが縦に伸びます。島の名前をダブルクリックするとその束だけが全面に広がり、`esc` で戻ります。

→ 仕組みとJSONの形は [GROUPS.md](GROUPS.md)
