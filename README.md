...
`tmux`が見つかる場合、セッションは`tmux new-session -A`で包んで起動します。フォルダと種類の組から決まる名前（`finderai-claude-xxxxxx`など）を持つため、アプリを終了・クラッシュしてもtmux側で作業が生き続け、次に同じフォルダでセッションを開始すると自動で再アタッチされます。

- サイドバーの「セッション」に、実行中（●）と保持中（⏸）が全フォルダ分並びます。クリックでそのフォルダへ移動できます。
- 保持中のセッションがあるフォルダのTerminalパネルには「再開」「破棄」が出ます。
- `⌘Q`時は「保持して終了」（tmuxに残す）と「すべて終了」（tmuxごと終了）を選べます。
- 普通のターミナルから`tmux attach -t <セッション名>`で同じ作業に入ることもできます。
- `tmux`が無い場合は従来どおりの直接起動で、永続化されません（自動インストールはしません）。
...
- セッションの永続化は`tmux`がインストール済みの場合のみ働きます（自動導入はしません）。tmux無しではアプリ終了・クラッシュとともにセッションも終了します。
- tmux画面のスクロールバックはtmux側（`Ctrl-b [`）の操作になります。
