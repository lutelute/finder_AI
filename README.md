...
[最新リリース](https://github.com/lutelute/finder_AI/releases/latest)から`FinderAI Workspace.zip`を落とし、`/Applications`へ入れてください。以降は自分で更新を取りに行きます。
...
1日に1度バックグラウンドで確認し、新しいバージョンがあれば**黙ってダウンロードして、アプリを終了したときに入れ替わります**。クリックは要りません。`FinderAI Workspace`メニューの「アップデートを確認…」でその場で確認・適用もできます。
...
ウインドウ位置・サイズ、サイドバー幅、並べ替え列と昇降順、Terminalの高さと開閉、隠しファイルの表示、最後に開いていたフォルダを次回起動時に復元します。最後のフォルダはbookmarkで保存するため、フォルダが移動・改名されても追従し、消えていればホームから開始します。
...
```
$ codesign -d -r- "/Applications/FinderAI Workspace.app"
...
- リスト表示とカラム表示のみで、アイコン／ギャラリー表示は未実装です。
